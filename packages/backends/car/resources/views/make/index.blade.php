...
<section class="content">
    ...
    <div class="card">
        <div class="card-header">
            <h3 class="card-title">{{ __('Make') }}</h3>
            <div class="card-tools">
                <a href="{{ route('make.create') }}" class="btn btn-tool" title="Remove">Add New</a>
            </div>
        </div>
                ...
                <tbody>
                    @foreach($makes as $item)
                        <tr>
                            ...
                            <td>
                                <a href="{{ route('make.show', $item->id) }}">
                                    {{ $item->name }}
                                </a>
                                <br />
                                <small>
                            ...
                            <td class="project-actions text-right">
                                <a class="btn btn-info btn-sm" href="{{ route('make.edit', $item->id) }}">
        ...
        <div class="card-footer clearfix">
            {{ $makes->links('backend/core::vendor.pagination.default') }}
        </div>
    </div>
    <!-- /.card -->
</section>
    ...
    <div class="modal-dialog" role="document">
        <div class="modal-content">
            ...
            <div class="modal-body">
                <p>Do you want to delete Make?</p>
            </div>
            <div class="modal-footer">
                ...
                <button type="button" class="btn btn-primary action_confirm" data-method="delete" onclick="if ($(this).hasClass('action_confirm')) { $(this).find('form').submit(); }">Yes
                    <form action="{{ route('make.destroy', 1) }}" method="post">
                        @method('DELETE')
                        {!! csrf_field() !!}
                    </form>
                </button>
