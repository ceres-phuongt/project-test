...
<section class="content">
    ...
    <div class="card">
        <div class="card-header">
            <h3 class="card-title">{{ __('Tag') }}</h3>
            <div class="card-tools">
                <a href="{{ route('tag.create') }}" class="btn btn-tool" title="Remove">Add New</a>
            </div>
        </div>
        ...
        <div class="card-footer clearfix">
            {{ $tags->links('backend/core::vendor.pagination.default') }}
        </div>
    </div>
    <!-- /.card -->
</section>
        ...
        <div class="modal-content">
            ...
            <div class="modal-footer">
                ...
                <button type="button" class="btn btn-primary action_confirm" data-method="delete" onclick="if ($(this).hasClass('action_confirm')) { $(this).find('form').submit(); }">Yes
                    <form action="{{ route('tag.destroy', 1) }}" method="post">
                        @method('DELETE')
                        {!! csrf_field() !!}
                    </form>
                </button>
